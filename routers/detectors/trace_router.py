...
# ── 分析结果缓存：同一 tx_hash 的多个面板端点共享一次完整分析 ──
# key = "chain_id:tx_hash", value = FullAnalysisResult
_analysis_cache: dict[str, "FullAnalysisResult"] = {}
_cache_locks: dict[str, asyncio.Lock] = {}   # 每个key一把锁，防同一tx并发重复分析
_cache_ttl_seconds: float = 300.0            # 缓存有效期 5 分钟
...
def _cache_key(tx_hash: str, chain_id: int) -> str:
    """生成缓存 key"""
...
async def _get_or_run_analysis(
    tx_hash: str,
    chain_id: int = 1,
    run_behavior_detect: bool = True,
) -> "FullAnalysisResult":
    """
    获取或执行分析（带结果缓存）。

    同一个 tx_hash + chain_id 的多个面板端点（call-tree / balance-changes /
    token-flows / behaviors / analyze）共享同一次完整分析结果，
    避免重复 RPC 调用（trace / receipt / build_tree 等）。
    """
    from detectors.trace.models import FullAnalysisResult

    key = _cache_key(tx_hash, chain_id)

    # 1) 命中有效缓存 → 直接返回
    cached = _analysis_cache.get(key)
    if cached is not None:
        cache_age = time.time() - cached.meta.get("analyzedAt", 0)
        if cache_age < _cache_ttl_seconds:
            logger.debug(
                f"[trace_router] Cache HIT for {tx_hash[:16]}... "
                f"(age={cache_age:.0f}s < TTL={_cache_ttl_seconds:.0f}s)"
            )
            return cached
        else:
            logger.debug(f"[trace_router] Cache expired for {tx_hash[:16]}...")
            del _analysis_cache[key]

    # 2) 缓存未命中 → 加锁保证同一 tx 只做一次完整分析
    lock = _cache_locks.setdefault(key, asyncio.Lock())
    async with lock:
        # double-check：等待锁期间可能已被其他协程写入
        cached = _analysis_cache.get(key)
        if cached is not None:
            return cached

        analyzer = _get_analyzer()
        logger.info(f"[trace_router] Running full analysis for {tx_hash[:16]}... (cache miss)")
        result = await analyzer.analyze(
            tx_hash=tx_hash,
            chain_id=chain_id,
            run_behavior_detect=run_behavior_detect,
        )

        _analysis_cache[key] = result

        # 清理过期条目（简单策略：超过 2 倍 TTL 的都清掉）
        now = time.time()
        expired_keys = [
            k for k, v in _analysis_cache.items()
            if now - v.meta.get("analyzedAt", 0) > _cache_ttl_seconds * 2
        ]
...
async def get_balance_changes(req: PanelRequest):
    ...
    try:
        ...
        return {
            ...
            "callTransfers": [c.to_dict() for c in result.call_transfers],
            "apiElapsedSeconds": elapsed,
        }
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
